...
<script type="text/babel">
  import { limitTimeRange, isDate, clearMilliseconds, timeWithinRange } from 'element-ui-lx/src/utils/date-util';
  ...
  export default {
    ...
    watch: {
      visible(val) {
        if (val) {
          if (!this.value && this.confirmControl) {
    ...
    computed: {
      ...
      useConfirm() {
        return this.confirmControl || false;
      },
      amPmMode() {
    ...
    methods: {
      ...
      handleChange(date) {
        ...
        if (this.visible) {
          ...
          if (this.isValidValue(this.date) && !this.confirmControl) {
